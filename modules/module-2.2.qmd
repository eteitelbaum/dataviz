---
title: "Color, Themes & Annotations"
...
The first way to make your plot more accessible is to create your own [discrete scale](https://ggplot2.tidyverse.org/reference/scale_manual.html) using `scale_color_manual()` or `scale_fill_manual()` (depending on what type of plot you are trying to draw).

Let's fix the line chart with the [Okabe-Ito palette](http://www.cookbook-r.com/Graphs/Colors_(ggplot2)/), a set of eight colors chosen specifically to stay distinguishable under color vision deficiency. First we create a vector of colors called `cb_palette`. Then we apply it to our lines with `scale_color_manual()`. We use `scale_color_manual()` rather than `scale_fill_manual()` because our chart is drawn with lines rather than filled shapes.

```{r}
#| label: dem_waves_manual
