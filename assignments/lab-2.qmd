...
*Use `vdem` from `vdemdata` to download the V-Dem dataset, then use `filter()` and `select()` to keep a democracy measure of your choice for all countries from roughly 1990 onward. Then use `wb_data()` from `wbstats` to download GDP per capita (or another development or women's-empowerment indicator) for the same period. `glimpse()` each to confirm they came through.*
